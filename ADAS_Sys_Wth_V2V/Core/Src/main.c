...
/*CAR_Directions_Backward();
SetMotorSpeed(SPEED_LEVEL_1);
SetMotorSpeed(SPEED_LEVEL_2);
SetMotorSpeed(SPEED_LEVEL_3);
SetMotorSpeed(SPEED_LEVEL_4);
SetMotorSpeed(SPEED_LEVEL_5);
*/
...
uint16_t d;
  ...
  while (1)
  {
	  ...
	  HAL_Delay(250);
	  */
	 APP_CONTROL_SYSTEM_RUNNABLE();
	 Prepare_Data_Tasks();
	 APP_Emergency_Braking_Task();
	 Is_Car_Forward();
	 APP_Adaptive_Cruise_Control_Task();
